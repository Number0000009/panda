...
static void framebuffer_fill(void)
{
...
static uint32_t pack_pos(uint32_t x, uint32_t y)
{
    return (y << 16) | x;
}
...
static void dispc_lcd2_config(void)
{
    ...
    writel(0x00ff0000u, DISPC_BASE + DISPC_DEFAULT_COLOR2);
...
static void dispc_vid1_config_disabled(void)
{
    ...
    writel(pack_pos(0, 0), DISPC_BASE + DISPC_VID1_POSITION);
